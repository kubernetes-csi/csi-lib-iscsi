...
func waitForPathToExist(logger klog.Logger, devicePath *string, maxRetries, intervalSeconds uint, deviceTransport string) error {
	...
	for i := uint(0); i <= maxRetries; i++ {
		...
		if err := pathExists(logger, devicePath, deviceTransport); err == nil {
			return nil
...
func GetConnectorFromFile(logger klog.Logger, filePath string) (*Connector, error) {
	...
	if devices, err := GetSCSIDevices(logger, []string{c.MountTargetDevice.GetPath()}, false); err != nil {
		return nil, err
	} else {
		c.MountTargetDevice = &devices[0]
	}

	if c.Devices, err = GetSCSIDevices(logger, devicePaths, false); err != nil {
		return nil, err
	}
